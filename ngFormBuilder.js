          ...
          '<accordion close-others="true">' +
            '<accordion-group ng-repeat="(groupName, group) in formComponentGroups" heading="{{ group.title }}" is-open="$first">' +
              '<div ng-repeat="component in formComponentsByGroup[groupName]" ng-if="component.title"' +
                ...
                '<button type="button" class="btn btn-primary btn-xs btn-block" disabled="disabled" style="pointer-events: none"><i ng-if="component.icon" class="{{ component.icon }}"></i> {{ component.title }}</button>' +
              '</div>' +
            '</accordion-group>' +
          '</accordion>' +
        '</div>' +
